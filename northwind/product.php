...
?>
  ...
  <div id="categories">
    <table id="product">
      ...
      <tbody>
<?php
$sql = 'select ProductID, ProductName, UnitPrice from products where CategoryID=?';
...
if(isset($_SESSION['user'])) {
  ...
  try {
    ...
    $row = $result->fetch_array(); //index 0,1  row0 = cartID, row1 = slip
    if($row) {
      if($row[1]) {
        echo '<h1>Slip</h1>';
...
?>
    ...
    for(let prod of document.querySelectorAll('.product')) {
      prod.addEventListener('click', function() {
        location.href = 'api/updatecart.php?category=<?=$_GET['category']?>&ProductID='+this.dataset.id;
      });
    }
<?php
}
